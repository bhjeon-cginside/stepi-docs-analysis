...
type_rows.append(tr([('<strong>합계</strong>',False),(fmt_num(sum(type_count.values())),True),(fmt_bytes(sum(type_size.values())),True)], 'total-row'))
...
md.append(f'| **합계** | **{fmt_num(sum(type_count.values()))}** | **{fmt_bytes(sum(type_size.values()))}** |')
