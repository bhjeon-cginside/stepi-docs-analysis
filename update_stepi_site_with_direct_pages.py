...
def display_type(s):
    return '압축파일' if clean(s) == 'Archive' else clean(s)
def safe_name(s):
...
css='''
:root{--fg:#1f2937;--muted:#6b7280;--line:#e5e7eb;--bg:#f8fafc;--card:#fff;--blue:#1d4ed8}*{box-sizing:border-box}body{margin:0;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,"Noto Sans KR",Arial,sans-serif;color:var(--fg);background:var(--bg)}header{padding:36px 28px;background:linear-gradient(135deg,#0f172a,#1d4ed8);color:white}header h1{margin:0 0 8px;font-size:32px}header p{margin:4px 0;opacity:.9}main{max-width:1180px;margin:0 auto;padding:24px}nav{background:#fff;border:1px solid var(--line);border-radius:14px;padding:14px 18px;margin-top:-22px;box-shadow:0 8px 18px rgba(15,23,42,.08);position:sticky;top:8px;z-index:2}nav a{margin-right:14px;color:var(--blue);text-decoration:none;font-weight:600;font-size:14px}section{margin:28px 0}h2{margin:0 0 14px;font-size:24px}h3{margin:18px 0 10px}.grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px}.stat{background:var(--card);border:1px solid var(--line);border-radius:14px;padding:16px}.stat .num{font-size:26px;font-weight:800;margin-top:6px}.stat .label{color:var(--muted);font-size:13px}table{width:100%;border-collapse:collapse;background:#fff;border:1px solid var(--line);border-radius:12px;overflow:hidden}th,td{border-bottom:1px solid var(--line);padding:9px 10px;text-align:left;vertical-align:top}th{background:#f1f5f9}td.num,th.num{text-align:right;font-variant-numeric:tabular-nums}.total-row td{font-weight:800;background:#f8fafc;border-top:2px solid #cbd5e1}.note{background:#fff7ed;border:1px solid #fed7aa;border-radius:12px;padding:14px;color:#7c2d12}.card{background:#fff;border:1px solid var(--line);border-radius:16px;padding:18px;margin:18px 0;box-shadow:0 2px 10px rgba(15,23,42,.04)}.badge{display:inline-block;padding:4px 8px;border-radius:999px;background:#dbeafe;color:#1e40af;font-size:12px;font-weight:700}.path{color:var(--muted);font-size:12px;word-break:break-all}.shots{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-top:14px}.shot{border:1px solid var(--line);border-radius:12px;padding:8px;background:#fafafa}.shot img{width:100%;border:1px solid #ddd;background:white;cursor:zoom-in;transition:transform .15s ease,box-shadow .15s ease}.shot img:hover{transform:translateY(-1px);box-shadow:0 8px 18px rgba(15,23,42,.14)}.shot .cap{font-size:12px;color:var(--muted);margin-bottom:6px}.lightbox{position:fixed;inset:0;display:none;align-items:center;justify-content:center;background:rgba(15,23,42,.86);z-index:50;padding:20px}.lightbox.open{display:flex}.lightbox-panel{position:relative;max-width:min(96vw,1280px);max-height:94vh;width:100%;display:flex;flex-direction:column;gap:10px}.lightbox-title{color:white;font-size:14px;line-height:1.45;word-break:break-word;padding-right:48px}.lightbox img{max-width:100%;max-height:84vh;object-fit:contain;background:white;border-radius:10px;box-shadow:0 24px 80px rgba(0,0,0,.45);cursor:zoom-out}.lightbox-close{position:absolute;right:0;top:-8px;width:40px;height:40px;border:0;border-radius:999px;background:white;color:#111827;font-size:26px;line-height:1;cursor:pointer}@media(max-width:860px){.grid{grid-template-columns:repeat(2,1fr)}nav{position:static;margin-top:0}.shots{grid-template-columns:repeat(2,minmax(0,1fr))}.lightbox{padding:10px}.lightbox img{max-height:80vh}}@media(max-width:520px){.shots{grid-template-columns:1fr}}
'''
def tr(cells, cls=''):
...
type_rows=[tr([(html.escape(display_type(k)),False),(fmt_num(c),True),(f'{(c/total_files*100):.1f}%',True),(fmt_bytes(type_size[k]),True)]) for k,c in type_count.most_common()]
...
for r in samples:
    ...
    doc_title=f'{clean(r.get("category"))} · {clean(r.get("year"))} · {fmt_num(r.get("pages"))}쪽'
    shots=''.join(f'<div class="shot"><div class="cap">p.{c["page"]}</div><img src="{html.escape(os.path.relpath(c["path"],DOCS))}" loading="lazy" alt="{html.escape(doc_title)} p.{c["page"]}" data-title="{html.escape(doc_title)} · p.{c["page"]}"></div>' for c in r.get('captures',[]) if c.get('ok'))
...
for k,c in type_count.most_common(): md.append(f'| {display_type(k)} | {fmt_num(c)} | {(c/total_files*100):.1f}% | {fmt_bytes(type_size[k])} |')
